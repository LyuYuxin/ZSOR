...
class RPNHead(AnchorHead):
    ...
    def _get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            rpn_cls_score = cls_scores[idx]#3 * 112 * 112
            rpn_bbox_pred = bbox_preds[idx]# 12 * 112 * 112
            assert rpn_cls_score.size()[-2:] == rpn_bbox_pred.size()[-2:]
            rpn_cls_score = rpn_cls_score.permute(1, 2, 0)#112 * 112 * 3
            if self.use_sigmoid_cls:
                rpn_cls_score = rpn_cls_score.reshape(-1) # 37632
            ...
            anchors = mlvl_anchors[idx] # 37632 * 4
            if cfg.nms_pre > 0 and scores.shape[0] > cfg.nms_pre:
                # sort is faster than topk
                # _, topk_inds = scores.topk(cfg.nms_pre)
                ranked_scores, rank_inds = scores.sort(descending=True)
                topk_inds = rank_inds[:cfg.nms_pre]
                scores = ranked_scores[:cfg.nms_pre] # 20000
        ...
        scores = torch.cat(mlvl_scores) # 5个尺度的所有anchor的分数
        anchors = torch.cat(mlvl_valid_anchors)# 6732
        rpn_bbox_pred = torch.cat(mlvl_bbox_preds)
        proposals = self.bbox_coder.decode(
            anchors, rpn_bbox_pred, max_shape=img_shape)# 6732 * 4
